...
namespace SimpleNethereum.Tests
{
    ...
    public class ClientTests
    {
        ...
        public async Task CallContract()
        {
            ...
            var counter = await client.Call("8ada066e", accounts[0], address, new HexBigInteger(BigInteger.Zero), new HexBigInteger(new BigInteger(1000000)), new HexBigInteger("0x00"));

            Assert.IsNotNull(counter);
            Assert.AreEqual(BigInteger.One, new HexBigInteger(counter).Value);
        }

        [TestMethod]
        public async Task InvokeContract()
        {
            var client = new Client();
            ...
            var hash = await client.Invoke("d09de08a", accounts[0], address, new HexBigInteger(BigInteger.Zero), new HexBigInteger(new BigInteger(1000000)), new HexBigInteger("0x00"));
            ...
            Assert.IsNotNull(counter);
            Assert.AreEqual(new BigInteger(2), new HexBigInteger(counter).Value);
        }
    }
}
